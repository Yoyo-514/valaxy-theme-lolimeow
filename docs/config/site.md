...
`siteConfig.lang` 用于选择主题固定 UI 文案的语言，例如搜索、分页、目录、404 按钮与辅助性标签。
...
文章内容、导航文本、公告、置顶标题等作者配置内容不会被主题自动翻译，请直接使用站点目标语言编写。主题不提供运行时语言切换按钮。

::: warning 修改 `lang` 后页面语言未变化？
Valaxy 会将访客的语言偏好持久化在浏览器 `localStorage` 的 `valaxy-locale` 键中，并在页面水合完成后优先于站点默认值生效。
因此修改 `lang` 后，回访浏览器仍可能展示旧语言。测试时请使用无痕窗口，或在浏览器控制台执行 `localStorage.removeItem('valaxy-locale')` 后刷新。
:::
